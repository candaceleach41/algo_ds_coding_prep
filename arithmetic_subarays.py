...
Explanation:
In the 0th query, the subarray is [4,6,5]. This can be rearranged as [6,5,4], which is
an arithmetic sequence.
In the 1st query, the subarray is [4,6,5,9]. This cannot be rearranged as an arithmetic
sequence.
In the 2nd query, the subarray is [5,9,3,7]. This can be rearranged as [3,5,7,9], which
...
def check_arithmetic_subarrays(nums, left, right):
    def check(arr):
        if len(arr) <= 2:
            return True
        diff = arr[1] - arr[0]
    ...
    for i in range(len(left)):
        result.append(check(sorted(nums[left[i]: right[i] + 1])))
    return result
