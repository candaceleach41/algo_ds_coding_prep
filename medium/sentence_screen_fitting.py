...
def words_typing(sentence, rows, cols):
    total_sentence = " ".join(sentence) + " "
    total_length = len(total_sentence)
    pointer = 0

    for _ in range(rows):
        pointer += cols
        if total_sentence[pointer % total_length] == " ":
            pointer += 1
        else:
            while pointer >= 0 and total_sentence[pointer % total_length] != " ":
                pointer -= 1
            pointer += 1

    return pointer // total_length
