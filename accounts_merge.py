...
def accounts_merge(accounts: List[List[str]]) -> List[List[str]]:
    ...
    for i in range(len(accounts)):
        name, account = accounts[i][0], accounts[i][1:]
        for email in account:
            if email not in emails:
                emails[email] = i
                graph[i]
            else:
                graph[i].append(emails[email])
                graph[emails[email]].append(i)
        if i not in graph:
            graph[i] = []
    ...
    for account in graph:
        if account not in visited:
            ...
            while stack:
                curr = stack.popleft()
                merged.extend(accounts[curr][1:])
                for n in graph[curr]:
                    if n not in visited:
                        visited.add(n)
                        stack.append(n)
            res.append([accounts[account][0]] + sorted(list(set(merged))))
