...
milestones where each milestones[i] denotes the number of milestones the ith project
has.
...
Note that you may not be able to finish every project's milestones due to these
constraints.
...
Note that you cannot work on the last milestone of project 0 on 8th week because it
...
def number_of_weeks(milestones):
    all_projects = sum(milestones)
    big_projects = max(milestones)
    remainder = all_projects - big_projects

    return min(all_projects, remainder * 2 + 1)
